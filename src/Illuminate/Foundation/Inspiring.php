...
class Inspiring
{
    ...
    {
        return static::format_for_console(static::quotes()->random());
    }
    /**
     ...
     * @param  string  $quote
     */
    protected static function format_for_console($quote): string
    {
        [$text, $author] = (new Stringable($quote))->explode('-');
        return sprintf("\n  <options=bold>“ %s ”</>\n  <fg=gray>— %s</>\n", trim($text), trim($author));
    }
}